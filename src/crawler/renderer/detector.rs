...
pub(crate) fn should_prefer_rendered_page(
    raw_page: &ExtractedPage,
    rendered_page: &ExtractedPage,
) -> bool {
    extraction_quality_score(rendered_page) > extraction_quality_score(raw_page)
}

fn extraction_quality_score(page: &ExtractedPage) -> usize {
    let mut score = 0;

    if !page.metadata.title.trim().is_empty() {
        score += 2;
    }
    if !page.metadata.meta_description.trim().is_empty() {
        score += 1;
    }
    if !page.headings.primary_h1.trim().is_empty() {
        score += 2;
    }
    score += if page.visible_text.len() >= MIN_VISIBLE_TEXT_LENGTH {
        3
    } else if !page.visible_text.trim().is_empty() {
        1
    } else {
        0
    };
    score += if page.links.len() > MIN_LINK_COUNT {
        2
    } else if !page.links.is_empty() {
        1
    } else {
        0
    };
...
mod tests {
    ...
    #[test]
    fn prefers_rendered_page_with_greater_extraction_quality() {
        let url = Url::parse("https://example.com/").unwrap();
        let raw = extract_page(b"<title>Raw</title>", &url);
        let rendered = extract_page(
            b"<title>Rendered</title><meta name=\"description\" content=\"Summary\">",
            &url,
        );

        assert!(should_prefer_rendered_page(&raw, &rendered));
    }

    #[test]
    fn retains_raw_page_when_extraction_quality_is_equal() {
        let url = Url::parse("https://example.com/").unwrap();
        let raw = extract_page(b"<title>Raw</title>", &url);
        let rendered = extract_page(b"<title>Rendered</title>", &url);

        assert!(!should_prefer_rendered_page(&raw, &rendered));
    }

    #[test]
    fn retains_raw_page_when_rendered_extraction_quality_is_worse() {
        let url = Url::parse("https://example.com/").unwrap();
        let raw = extract_page(b"<title>Raw</title>", &url);
        let rendered = extract_page(b"", &url);
