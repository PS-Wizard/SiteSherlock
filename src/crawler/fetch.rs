...
use crate::crawler::{
    ...
    renderer::{RenderPool, needs_js_render, should_prefer_rendered_page},
...
pub(crate) async fn fetch_url_with_renderer(
    ...
    let mut response_size = raw.body.len();
            ...
            .is_some_and(is_html_content_type)
    {
        ...
        let page = if let Some(renderer) =
            renderer.filter(|_| needs_js_render(&raw.final_url, &raw.body, &raw_page).needs_render)
        {
            ...
            match rendered_result {
                ...
                {
                    let rendered_response_size = rendered.html.len();
                    let rendered_response_time = rendered.response_time;
                    ...
                    if should_prefer_rendered_page(&raw_page, &rendered_page) {
                        response_size = rendered_response_size;
                        javascript_render_time = rendered_response_time;
