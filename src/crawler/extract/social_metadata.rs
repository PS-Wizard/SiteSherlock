    ...
    for element in document.select(&SOCIAL_METADATA_SELECTOR) {
        ...
        if let Some(property) = element.value().attr("property").map(str::trim)
            && (property
                ...
                || property.eq_ignore_ascii_case("article:author"))
        {
            social_metadata
                .open_graph
...
mod tests {
    ...
    #[test]
    fn preserves_case_insensitive_article_author_properties() {
        assert_eq!(
            extract(r#"<meta property="ARTICLE:AUTHOR" content=" Jane Doe ">"#).open_graph,
            HashMap::from([("ARTICLE:AUTHOR".to_owned(), "Jane Doe".to_owned())]),
        );
    }
